...
Update OneMCP to latest version.
...
- Updates CLI if installed via Homebrew/Scoop
...
**Note:** If installed via Homebrew/Scoop, use their update commands instead:
```bash
brew upgrade onemcp
# or
scoop update onemcp
...
└── openapi/              # OpenAPI specs directory
    └── README.md         # Instructions for adding specs
```
...
└── data/                 # Data files (optional, not used by server)
```
