---
description: Run MCP Agent with Docker using various process modes and configurations.
---
...
Starts MCP server + orchestrator:
```bash
...
For local development, you can run the container in dependencies mode to start only the supporting services while running the main MCP Agent application locally.
